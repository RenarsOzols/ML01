...
class MassEnable extends Action implements HttpPostActionInterface
{
    ...
    public function execute(): ResultInterface
    {
        try {
            ...
            $this->messageManager->addErrorMessage(__('An error occurred while enabling the records. Please try again later.'));
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('*/*/');
        }
        foreach ($collection as $item) {
            $this->questionManagement->enableQuestion((int)$item);
        }

        $this->messageManager->addSuccessMessage(
            __('A total of %1 record(s) have been enabled.', count($collection))
        );
